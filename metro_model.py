...
import sys
sys.path.append('/home/liqing/MORL/Metro-Line/Metro-Line/')
...
class DRL4Metro(nn.Module):  
    ...
    def __init__(self, static_size, dynamic_size, hidden_size, update_fn = None, mask_fn = None, v_to_g_fn = None, vector_allow_fn = None, num_layers=1, dropout=0.):
    ...
    def forbid_overlap(self, pre_station, current_allow):
        existing_lines = [[234, 293, 295, 325, 326, 357, 359, 360, 361, 362, 363, 364, 365, 366, 368, 341, 342, 343, 344], [13, 43, 101, 130, 159, 188, 246, 275, 304, 362, 391, 420, 449, 478, 507, 536, 594, 623, 681, 710, 768],[113,169,196,195,224,223,222,251,280,279,308,337,336,365,394,393,392,391,390,419,448,447,476,475,504,503,502,531,560,559,558,587,616,645,644],[145,203,233,234,263,264,265,294,323,324,325,354,355,384,385,414,415,416,445,446,475,505,506,507,508,537,538,567,568,597,598,599,628,629,658]]

        #existing_lines = [[234, 293, 295, 325, 326, 357, 359, 360, 361, 362, 363, 364, 365, 366, 368, 341, 342, 343, 344], [13, 43, 101, 130, 159, 188, 246, 275, 304, 362, 391, 420, 449, 478, 507, 536, 594, 623, 681, 710, 768]]


        for i in range(len(existing_lines)):
            if pre_station in existing_lines[i]:
                existing_line = torch.tensor(existing_lines[i]).to(device)
                tensor_is_not_in = torch.isin(current_allow,existing_line,invert=True)
                current_allow = current_allow[tensor_is_not_in]          
                pass
        return current_allow




    def forward(self, static, dynamic, allowed, station_num_lim, specify_original_station = 0, budget =None, initial_direct = None,line_unit_price = None, station_price = None, decoder_input=None, last_hh=None):
        ...
        if allowed:
            allowed = torch.tensor(allowed).to(device)
            mask_initial = torch.zeros(1, 841, device=device).long() 
            mask = mask_initial.index_fill (1, allowed, 1).float()
        ...
        for _ in range(max_steps):
            ...
            if len(allowed):
                ...
                vector_index_allow = torch.tensor(list(set(list(allowed.cpu().detach().numpy())) & set(list(vector_index_allow.cpu().detach().numpy())))).to(device)
